...
-- A function set up by drag-to-pan. Must be called to clean lingering state.
local cleanup = nil
...
function cursor_centric_zoom_handler(amt)
  ...
  then
    -- The zoom decrement is too much, reduce it so that the full image is
    -- visible, no more, no less. In addition, this should take care of trying
    -- to zoom out while everything is already visible.
    local new_zoom_inc_x = math.log((dim.w - 2 * margin) / video_size[1])
                         / math.log(2)
    local new_zoom_inc_y = math.log((dim.h - 2 * margin) / video_size[2])
                         / math.log(2)
  ...
  -- If border would show on either side, then prefer adjusting the pan even
  -- if not cursor-centric.
  ...
  if auto_c and video_size[1] + diff_width + 2 * margin <= dim.w then
    ...
    local rx = (dim.ml + video_size[1] / 2 - mouse_pos_origin[1])
             / (video_size[1] / 2)
  ...
  if auto_c and video_size[2] + diff_height + 2 * margin <= dim.h then
    ...
    local ry = (dim.mt + video_size[2] / 2 - mouse_pos_origin[2])
             / (video_size[2] / 2)
